...
func (suite *ScannerTestSuite) SetupTest() {
	...
	suite.s = New(config.NewLogSources(), suite.openFilesLimit, suite.pipelineProvider, auditor.NewRegistry(), sleepDuration)
...
func TestScannerScanStartNewTailer(t *testing.T) {
	...
	openFilesLimit := 2
	sleepDuration := 20 * time.Millisecond
	scanner := New(config.NewLogSources(), openFilesLimit, mock.NewMockProvider(), auditor.NewRegistry(), sleepDuration)
