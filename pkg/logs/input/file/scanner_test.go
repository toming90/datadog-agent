...
func (suite *ScannerTestSuite) SetupTest() {
	...
	suite.s = New(sources, suite.openFilesLimit, suite.pipelineProvider, auditor.NewRegistry(), sleepDuration)
	suite.s.setup()
}
...
func TestScannerScanStartNewTailer(t *testing.T) {
	...
	scanner := New(sources, openFilesLimit, mock.NewMockProvider(), auditor.NewRegistry(), sleepDuration)
